 ...
 * Created at 3/21/18
 *
 * This file sm4_cipher.c is part of ccs_engine.
 *
...
static const uint32_t
    fkexp[] = {0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc};

static const uint32_t ckexp[] =
...
/**
 * substitute byte through sbox
 * @param in
 *      2 hex to be substituted
 * @param flag
 *      1 for dec/encryption, 0 for key schedule
 * @return
 *      2 hex after substitution, we need function to return the WORD,
 *      so other functions don't complaint sub_word does not return anything
 *      in c-style expr.
 */
static uint32_t
sub_word(uint32_t in, int flag)
{
    uint8_t *in_8 = (uint8_t *) &in;
    in_8[0] = sbox[in_8[0] >> 4][in_8[0] & 0xf];
    in_8[1] = sbox[in_8[1] >> 4][in_8[1] & 0xf];
    in_8[2] = sbox[in_8[2] >> 4][in_8[2] & 0xf];
    in_8[3] = sbox[in_8[3] >> 4][in_8[3] & 0xf];
...
sm4_init(sm4_ctx_t *stx, const uint8_t *k8)
{
    ...
    {
        stx->k[i + 4] =
            stx->k[i] ^ sub_word(stx->k[i + 1] ^ stx->k[i + 2] ^ stx->k[i + 3] ^ ckexp[i], 0);
    }
}
...
{
    uint32_t in32[4], out32[4];
    u8_to_u32_list(in, 16, in32);
    ...
    {
        x[i + 4] =
            x[i] ^ sub_word(x[i + 1] ^ x[i + 2] ^ x[i + 3] ^ stx->k[i + 4], 1);
    }
    ...
    out32[2] = x[33];
    out32[3] = x[32];
    u32_to_u8_list(out32, 4, out);
}

void
decrypt_block_sm4_ecb(sm4_ctx_t *stx, uint8_t in[16], uint8_t out[16])
{
    uint32_t in32[4], out32[4];
    u8_to_u32_list(in, 16, in32);
    ...
    {
        x[i + 4] =
            x[i] ^ sub_word(x[i + 1] ^ x[i + 2] ^ x[i + 3] ^ stx->k[35 - i], 1);
    }
